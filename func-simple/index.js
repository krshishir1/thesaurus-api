  ...
  try {
    ...
    if (items.length)
      return (context.res = {
        status: 200,
        headers,
        body: simplifyData(items[0]),
      });

    let response = await extractData(wordQuery);

    if (response.status === 200) {
      context.res = {
        status: response.status,
        headers,
        body: simplifyData({ word: wordQuery, data: response.body }),
      };
...
const simplifyData = function (wordsObj) {
  wordsObj.data = wordsObj.data.map((el) => {
    let obj = {};

    for (const prop in el) {
      if (prop === "word") {
        obj["context"] = el[prop];
      } else obj[prop] = el[prop];
    }
