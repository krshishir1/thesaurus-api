...
module.exports = async function (context, req) {
  ...
  try {
    ...
    if (!wordQuery) return context.res = { status: 400, headers, body: { code: "QUERY_NOT_FOUND" } };
    ...
    if(!items) return context.res = { status: 500, headers, body: { code: "INTERNAL_SERVER_ERROR" } };
    if(items.length) return context.res = { status: 200, headers, body: getSynonymsData(items[0]) }
    ...
    context.res = {
        status, headers,
        body: getSynonymsData({word: wordQuery, data: body})
    }
    ...
    return context.res = { status: 500, headers, body: { code: "INTERNAL_SERVER_ERROR" } }
  }
    
}

const getSynonymsData = function (arr) {
    ...
    for (let i = 0; i < arr.data.length; i++) {
        ...
        const el = arr.data[i]
        const wordChunk = {context: el.word, type: el.type, synonyms: []}
        
        if(el.synonyms !== "Data not available") {
            el.synonyms.split(",").forEach(chunk => {
                if(chunk) wordChunk.synonyms.push(chunk)
            });
        } else wordChunk.synonyms = el.synonyms
    ...
    try {
      ...
      if (typeSections.length) {
        typeSections.each((i, el) => {
          let partsOfSpeech = $(el).find("h2.parts-of-speech").first().text().replace(/\(.+\)/g, "").trim()
